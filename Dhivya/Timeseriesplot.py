...
def plot_time_series_with_anomalies(data, column, iqr_outliers,␣
↪zscore_outliers):
plt.figure(figsize=(14, 6))
# Plot the time series with a light blue color
plt.plot(data.index, data[column], label='Data', color='lightblue', alpha=0.
↪7)
34
# Plot IQR outliers with dark blue color
plt.scatter(iqr_outliers.index, iqr_outliers[column], color='darkblue',␣
↪label='IQR Outliers', s=50, marker='o')
# Plot Z-score outliers with bright red color
plt.scatter(zscore_outliers.index, zscore_outliers[column], color='red',␣
↪label='Z-Score Outliers', s=50, marker='x')
...
columns = ['Acc X','Acc Y','Acc␣
↪Z','Heading','gyro_x','gyro_y','Gyro_Change','Net_Displacement','Speed_Change',
␣
↪'Heading_Change','Rolling_Acc_Mean','Rolling_Acc_STD','acc_mean','acc_std','gyro_mean','gyrfor col in columns:
